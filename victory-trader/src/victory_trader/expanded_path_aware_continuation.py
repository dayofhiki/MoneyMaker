...
def build_continuation_rows(
    ...
    required = set(KEYS + ["t", "o", "h", "l", "c"])
    if not required.issubset(state.columns):
        raise ValueError("state panel requires trading_day/ticker/t/o/h/l/c")
    ...
    rows = rows.sort_values(KEYS + ["t"], kind="stable").reset_index(drop=True)
    ...
    rows["entry_reference_t"] = pd.to_numeric(rows["anchor_t"], errors="coerce") + MINUTE_MS
    entry_open = opens.rename(columns={"t": "entry_reference_t", "o": "entry_open"})
    ...
    rows = rows.merge(entry_open, on=KEYS + ["entry_reference_t"], how="left", validate="many_to_one")
    ...
    # v3.5 adds only causal position-path context observable at the decision open.
    # For a row timestamped t, the completed bar t and the open at t+1 are known.
    groupers = [rows[key].astype(str) for key in KEYS]
    entry_open_num = pd.to_numeric(rows["entry_open"], errors="coerce")
    decision_open = pd.to_numeric(rows["exit_open"], errors="coerce")
    high = pd.to_numeric(rows["h"], errors="coerce")
    low = pd.to_numeric(rows["l"], errors="coerce")
    timestamp = pd.to_numeric(rows["t"], errors="coerce")

    running_high = high.groupby(groupers, sort=False).cummax()
    running_low = low.groupby(groupers, sort=False).cummin()
    rows["path_entry_return_pct"] = (decision_open / entry_open_num - 1.0) * 100.0
    rows["path_mfe_pct"] = (running_high / entry_open_num - 1.0) * 100.0
    rows["path_mae_pct"] = (running_low / entry_open_num - 1.0) * 100.0
    rows["path_drawdown_from_high_pct"] = (decision_open / running_high - 1.0) * 100.0
    rows["path_rebound_from_low_pct"] = (decision_open / running_low - 1.0) * 100.0
    rows["path_range_pct"] = (running_high / running_low - 1.0) * 100.0

    high_tol = np.maximum(running_high.abs() * 1e-12, 1e-12)
    low_tol = np.maximum(running_low.abs() * 1e-12, 1e-12)
    new_high = high.notna() & high.ge(running_high - high_tol)
    new_low = low.notna() & low.le(running_low + low_tol)
    high_t = timestamp.where(new_high).groupby(groupers, sort=False).ffill()
    low_t = timestamp.where(new_low).groupby(groupers, sort=False).ffill()
    rows["path_minutes_since_high"] = (timestamp - high_t) / MINUTE_MS
    rows["path_minutes_since_low"] = (timestamp - low_t) / MINUTE_MS

    observed_rows = rows.groupby(KEYS, sort=False).cumcount() + 1
    held = pd.to_numeric(rows["minutes_held"], errors="coerce")
    rows["path_observed_fraction"] = (observed_rows / held).clip(upper=1.0)

    abs_one_minute = pd.to_numeric(
        rows["trailing_return_1m_pct"], errors="coerce"
    ).abs().fillna(0.0)
    path_abs_return = abs_one_minute.groupby(groupers, sort=False).cumsum()
    rows["path_efficiency"] = (
        rows["path_entry_return_pct"] / path_abs_return.replace(0.0, np.nan)
    )
    return rows
...
def render_report(outputs: tuple[pd.DataFrame, ...]) -> str:
    ...
    lines = [
        "=== MoneyMaker Path-Aware Continuation Observability v3.5 ===",
        ...
        "post-entry states held 1-29 minutes; entry-relative causal path context added",
        "no missing-bar fallback; unchanged model/label/threshold; April 2026+ sealed",
    ]
