from __future__ import annotations

import argparse
import heapq
from dataclasses import dataclass
from pathlib import Path

import numpy as np
import pandas as pd
from sklearn.ensemble import HistGradientBoostingRegressor

from .state_action_risk import SEVERE_LOSS_PCT
from .state_action_value import (
    HORIZONS,
    _fit_affine_calibration,
    _sample_training,
    _target_column,
    action_feature_frame,
    score_actions,
    train_direct_ev_model,
)
from .state_value_model import (
    _chronological_fit_calibration_split,
    _eligible,
)


EV_GATE_PCT = 0.50
WAIT_WINDOW_MINUTES = 15
MINUTE_MS = 60_000
WAIT_WINDOW_MS = WAIT_WINDOW_MINUTES * MINUTE_MS
EPISODE_KEYS = ["trading_day", "ticker"]
MIN_MONTH_TRADES = 15
TARGET_WINSOR_LOW = 0.005
TARGET_WINSOR_HIGH = 0.995
POLICIES = ("earliest_ev_cap1", "wait_value_cap1")
BOOTSTRAP_SAMPLES = 10_000


@dataclass(frozen=True)
class WaitValueModel:
    model: HistGradientBoostingRegressor
    feature_columns: tuple[str, ...]
    target_low: float
    target_high: float
    calibration_intercept: float
    calibration_slope: float


def current_realized_action_value(frame: pd.DataFrame) -> pd.Series:
    values = pd.DataFrame(
        {
            horizon: pd.to_numeric(
                frame[_target_column(horizon)],
                errors="coerce",
            )
            for horizon in HORIZONS
        },
        index=frame.index,
    )
    return values.max(axis=1, skipna=True)


def wait_advantage_target(
    frame: pd.DataFrame,
    *,
    wait_window_minutes: int = WAIT_WINDOW_MINUTES,
) -> pd.Series:
    """Realized value of waiting, used only as a supervised label.

    For each state, compare the best realized buy action available now with the
    best realized buy action available at any strictly later state inside the
    fixed wait window. All future-state access stays inside this target builder.
    """

    target = pd.Series(np.nan, index=frame.index, dtype=float)
    current = current_realized_action_value(frame)
    window_ms = int(wait_window_minutes * MINUTE_MS)

    work = frame.loc[:, EPISODE_KEYS + ["t"]].copy()
    work["_current_value"] = current

    for _, group in work.groupby(EPISODE_KEYS, sort=False):
        ordered = group.sort_values("t", kind="stable")
        indices = ordered.index.to_numpy()
        times = pd.to_numeric(ordered["t"], errors="coerce").to_numpy(
            dtype=float
        )
        values = pd.to_numeric(
            ordered["_current_value"], errors="coerce"
        ).to_numpy(dtype=float)
        future_best = np.full(len(ordered), np.nan, dtype=float)

        # Max-heap implemented as negative values. Walking backward lets us add
        # exactly the next strictly-future state while lazily removing states
        # that lie beyond the fixed wait window.
        heap: list[tuple[float, int]] = []
        for i in range(len(ordered) - 1, -1, -1):
            j = i + 1
            if j < len(ordered) and np.isfinite(values[j]):
                heapq.heappush(heap, (-float(values[j]), j))

            if not np.isfinite(times[i]):
                continue
            upper = float(times[i]) + window_ms
            while heap and (
                not np.isfinite(times[heap[0][1]])
                or float(times[heap[0][1]]) > upper
            ):
                heapq.heappop(heap)

            if heap:
                future_best[i] = -heap[0][0]

        valid = np.isfinite(values) & np.isfinite(future_best)
        episode_target = np.full(len(ordered), np.nan, dtype=float)
        episode_target[valid] = future_best[valid] - values[valid]
        target.loc[indices] = episode_target

    return target


def train_wait_value_model(train: pd.DataFrame) -> WaitValueModel:
    scoreable = train.loc[_eligible(train)].copy()
    scoreable["_wait_advantage_target"] = wait_advantage_target(scoreable)
    fit_period, calibration = _chronological_fit_calibration_split(scoreable)

    fit = fit_period.loc[
        pd.to_numeric(
            fit_period["_wait_advantage_target"], errors="coerce"
        ).notna()
    ].copy()
    fit = _sample_training(fit)
    if fit.empty:
        raise ValueError("no training labels for wait-value model")

    features = action_feature_frame(fit)
    columns = [
        column for column in features.columns if features[column].notna().any()
    ]
    if not columns:
        raise ValueError("no usable wait-value features")

    y = pd.to_numeric(fit["_wait_advantage_target"], errors="coerce")
    low = float(y.quantile(TARGET_WINSOR_LOW))
    high = float(y.quantile(TARGET_WINSOR_HIGH))
    y_fit = y.clip(lower=low, upper=high)

    model = HistGradientBoostingRegressor(
        loss="squared_error",
        learning_rate=0.05,
        max_iter=180,
        max_leaf_nodes=31,
        min_samples_leaf=300,
        l2_regularization=2.0,
        random_state=20261104,
    )
    model.fit(features.loc[:, columns], y_fit)

    cal = calibration.loc[
        pd.to_numeric(
            calibration["_wait_advantage_target"], errors="coerce"
        ).notna()
    ].copy()
    if cal.empty:
        intercept, slope = 0.0, 1.0
    else:
        raw_prediction = model.predict(
            action_feature_frame(cal).reindex(columns=columns)
        )
        actual = pd.to_numeric(
            cal["_wait_advantage_target"], errors="coerce"
        ).to_numpy(dtype=float)
        intercept, slope = _fit_affine_calibration(raw_prediction, actual)

    return WaitValueModel(
        model=model,
        feature_columns=tuple(columns),
        target_low=low,
        target_high=high,
        calibration_intercept=intercept,
        calibration_slope=slope,
    )


def score_wait_value(
    frame: pd.DataFrame,
    fitted: WaitValueModel,
) -> pd.Series:
    scoreable = frame.loc[_eligible(frame)].copy()
    raw = fitted.model.predict(
        action_feature_frame(scoreable).reindex(
            columns=fitted.feature_columns
        )
    )
    calibrated = fitted.calibration_intercept + fitted.calibration_slope * raw
    return pd.Series(
        calibrated,
        index=scoreable.index,
        name="predicted_wait_advantage_pct",
    )


def score_entry_states(
    frame: pd.DataFrame,
    direct_models: dict[int, object],
    wait_model: WaitValueModel,
) -> pd.DataFrame:
    scored = score_actions(frame, direct_models)
    scored["predicted_wait_advantage_pct"] = score_wait_value(
        frame, wait_model
    ).reindex(scored.index)
    return scored


def _policy_signal_mask(
    scored: pd.DataFrame,
    *,
    policy: str,
) -> pd.Series:
    ev_ok = pd.to_numeric(
        scored["predicted_best_base_ev_pct"], errors="coerce"
    ).ge(EV_GATE_PCT)
    if policy == "earliest_ev_cap1":
        return ev_ok
    if policy == "wait_value_cap1":
        wait = pd.to_numeric(
            scored["predicted_wait_advantage_pct"], errors="coerce"
        )
        return ev_ok & wait.le(0.0)
    raise ValueError(f"unknown wait-value policy: {policy}")


def _select_first_trades(
    scored: pd.DataFrame,
    *,
    policy: str,
) -> pd.DataFrame:
    signals = scored.loc[_policy_signal_mask(scored, policy=policy)].copy()
    if signals.empty:
        return signals

    trades: list[dict[str, object]] = []
    for _, group in signals.groupby(EPISODE_KEYS, sort=False):
        row = group.sort_values("t", kind="stable").iloc[0]
        horizon = int(row["predicted_best_horizon_min"])
        entry = pd.to_numeric(
            pd.Series([row.get("entry_price", np.nan)]), errors="coerce"
        ).iloc[0]
        realized = pd.to_numeric(
            pd.Series([row.get(_target_column(horizon), np.nan)]),
            errors="coerce",
        ).iloc[0]

        # The accepted signal consumes the episode. Never fall through to a
        # later row because the realized label/fill of the first row is absent.
        if pd.isna(entry) or float(entry) <= 0 or pd.isna(realized):
            continue

        item = row.to_dict()
        item["action_horizon_min"] = horizon
        item["selected_predicted_base_ev_pct"] = float(
            row["predicted_best_base_ev_pct"]
        )
        item["selected_predicted_wait_advantage_pct"] = float(
            row["predicted_wait_advantage_pct"]
        )
        item["realized_base_net_return_pct"] = float(realized)
        item["realized_gross_return_pct"] = float(
            row[f"buy_return_{horizon}m_pct"]
        )
        item["realized_stress_net_return_pct"] = float(
            row[f"buy_return_{horizon}m_stress_net_return_pct"]
        )
        trades.append(item)
    return pd.DataFrame(trades)


def _metrics(
    trades: pd.DataFrame,
    *,
    month: str,
    policy: str,
) -> dict[str, object]:
    if trades.empty:
        return {
            "month": month,
            "policy": policy,
            "trades": 0,
        }

    base = pd.to_numeric(
        trades["realized_base_net_return_pct"], errors="coerce"
    )
    gross = pd.to_numeric(
        trades["realized_gross_return_pct"], errors="coerce"
    )
    stress = pd.to_numeric(
        trades["realized_stress_net_return_pct"], errors="coerce"
    )
    daily = trades.assign(_base=base).groupby("trading_day")["_base"].mean()
    horizons = pd.to_numeric(trades["action_horizon_min"], errors="coerce")
    return {
        "month": month,
        "policy": policy,
        "trades": int(len(trades)),
        "ticker_day_episodes": int(
            trades[EPISODE_KEYS].drop_duplicates().shape[0]
        ),
        "days": int(trades["trading_day"].nunique()),
        "trades_per_day": float(
            len(trades) / trades["trading_day"].nunique()
        ),
        "predicted_base_ev_mean_pct": float(
            pd.to_numeric(
                trades["selected_predicted_base_ev_pct"], errors="coerce"
            ).mean()
        ),
        "predicted_wait_advantage_mean_pct": float(
            pd.to_numeric(
                trades["selected_predicted_wait_advantage_pct"],
                errors="coerce",
            ).mean()
        ),
        "gross_mean_pct": float(gross.mean()),
        "base_mean_pct": float(base.mean()),
        "base_median_pct": float(base.median()),
        "base_positive_rate": float(base.gt(0).mean()),
        "actual_severe_loss_rate": float(base.le(SEVERE_LOSS_PCT).mean()),
        "base_p05_pct": float(base.quantile(0.05)),
        "stress_mean_pct": float(stress.mean()),
        "day_balanced_base_mean_pct": float(daily.mean()),
        "worst_day_mean_pct": float(daily.min()),
        "action_5m_rate": float(horizons.eq(5).mean()),
        "action_10m_rate": float(horizons.eq(10).mean()),
        "action_15m_rate": float(horizons.eq(15).mean()),
        "median_entry_minute": float(
            pd.to_numeric(
                trades["minutes_since_10pct_cross"], errors="coerce"
            ).median()
        ),
    }


def _wait_diagnostic_row(
    scored: pd.DataFrame,
    *,
    month: str,
    pool: str,
    mask: pd.Series,
) -> dict[str, object]:
    actual = wait_advantage_target(scored)
    prediction = pd.to_numeric(
        scored["predicted_wait_advantage_pct"], errors="coerce"
    )
    valid = mask & actual.notna() & prediction.notna()
    y = actual.loc[valid]
    p = prediction.loc[valid]

    if len(y) >= 2:
        spearman = float(p.corr(y, method="spearman"))
    else:
        spearman = np.nan

    sign_agreement = (
        float(p.le(0).eq(y.le(0)).mean()) if len(y) else np.nan
    )
    return {
        "month": month,
        "pool": pool,
        "rows": int(len(y)),
        "predicted_mean_pct": float(p.mean()) if len(p) else np.nan,
        "realized_mean_pct": float(y.mean()) if len(y) else np.nan,
        "spearman": spearman,
        "sign_agreement": sign_agreement,
        "predicted_wait_rate": float(p.gt(0).mean()) if len(p) else np.nan,
        "realized_wait_rate": float(y.gt(0).mean()) if len(y) else np.nan,
    }


def _wait_diagnostics(
    scored: pd.DataFrame,
    *,
    month: str,
) -> pd.DataFrame:
    all_mask = pd.Series(True, index=scored.index)
    ev_mask = pd.to_numeric(
        scored["predicted_best_base_ev_pct"], errors="coerce"
    ).ge(EV_GATE_PCT)
    return pd.DataFrame(
        [
            _wait_diagnostic_row(
                scored, month=month, pool="all_states", mask=all_mask
            ),
            _wait_diagnostic_row(
                scored, month=month, pool="ev_qualified", mask=ev_mask
            ),
        ]
    )


def _common_episode_comparison(
    baseline: pd.DataFrame,
    waited: pd.DataFrame,
    *,
    month: str,
) -> dict[str, object]:
    if baseline.empty or waited.empty:
        return {"month": month, "common_episodes": 0}
    columns = EPISODE_KEYS + [
        "t",
        "realized_base_net_return_pct",
        "minutes_since_10pct_cross",
    ]
    merged = baseline.loc[:, columns].merge(
        waited.loc[:, columns],
        on=EPISODE_KEYS,
        suffixes=("_baseline", "_wait"),
    )
    if merged.empty:
        return {"month": month, "common_episodes": 0}

    delta = (
        merged["realized_base_net_return_pct_wait"]
        - merged["realized_base_net_return_pct_baseline"]
    )
    minute_delta = (
        merged["minutes_since_10pct_cross_wait"]
        - merged["minutes_since_10pct_cross_baseline"]
    )
    return {
        "month": month,
        "common_episodes": int(len(merged)),
        "wait_minus_baseline_base_mean_pct": float(delta.mean()),
        "wait_better_rate": float(delta.gt(0).mean()),
        "wait_later_rate": float(minute_delta.gt(0).mean()),
        "median_entry_delay_minutes": float(minute_delta.median()),
    }


def run_lomo(
    monthly_frames: dict[str, pd.DataFrame],
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    metric_rows: list[dict[str, object]] = []
    trade_frames: list[pd.DataFrame] = []
    diagnostics: list[pd.DataFrame] = []
    comparisons: list[dict[str, object]] = []

    for holdout_month, holdout in monthly_frames.items():
        train = pd.concat(
            [
                frame
                for month, frame in monthly_frames.items()
                if month != holdout_month
            ],
            ignore_index=True,
        )
        direct_models = {
            horizon: train_direct_ev_model(train, horizon)
            for horizon in HORIZONS
        }
        wait_model = train_wait_value_model(train)
        scored = score_entry_states(holdout, direct_models, wait_model)
        diagnostics.append(
            _wait_diagnostics(scored, month=holdout_month)
        )

        selected: dict[str, pd.DataFrame] = {}
        for policy in POLICIES:
            trades = _select_first_trades(scored, policy=policy)
            selected[policy] = trades
            metric_rows.append(
                _metrics(
                    trades,
                    month=holdout_month,
                    policy=policy,
                )
            )
            if not trades.empty:
                item = trades.copy()
                item["month"] = holdout_month
                item["policy"] = policy
                trade_frames.append(item)

        comparisons.append(
            _common_episode_comparison(
                selected["earliest_ev_cap1"],
                selected["wait_value_cap1"],
                month=holdout_month,
            )
        )

    return (
        pd.DataFrame(metric_rows),
        pd.concat(trade_frames, ignore_index=True)
        if trade_frames
        else pd.DataFrame(),
        pd.concat(diagnostics, ignore_index=True)
        if diagnostics
        else pd.DataFrame(),
        pd.DataFrame(comparisons),
    )


def summarize(details: pd.DataFrame) -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    for policy, group in details.groupby("policy", sort=True):
        valid = group.loc[group["trades"].ge(MIN_MONTH_TRADES)].copy()
        rows.append(
            {
                "policy": policy,
                "months_tested": int(len(valid)),
                "min_trades": int(valid["trades"].min())
                if len(valid)
                else 0,
                "base_positive_months": int(
                    valid["base_mean_pct"].gt(0).sum()
                ),
                "day_positive_months": int(
                    valid["day_balanced_base_mean_pct"].gt(0).sum()
                ),
                "median_base_mean_pct": float(
                    valid["base_mean_pct"].median()
                )
                if len(valid)
                else np.nan,
                "worst_base_mean_pct": float(
                    valid["base_mean_pct"].min()
                )
                if len(valid)
                else np.nan,
                "median_day_balanced_base_mean_pct": float(
                    valid["day_balanced_base_mean_pct"].median()
                )
                if len(valid)
                else np.nan,
                "median_base_p05_pct": float(
                    valid["base_p05_pct"].median()
                )
                if len(valid)
                else np.nan,
                "median_stress_mean_pct": float(
                    valid["stress_mean_pct"].median()
                )
                if len(valid)
                else np.nan,
            }
        )
    return pd.DataFrame(rows).sort_values(
        [
            "base_positive_months",
            "day_positive_months",
            "median_base_mean_pct",
            "worst_base_mean_pct",
        ],
        ascending=[False, False, False, False],
    ).reset_index(drop=True)


def day_cluster_bootstrap(
    trades: pd.DataFrame,
    *,
    policy: str = "wait_value_cap1",
    samples: int = BOOTSTRAP_SAMPLES,
) -> dict[str, float | int]:
    selected = trades.loc[trades["policy"].eq(policy)].copy()
    if selected.empty:
        return {
            "days": 0,
            "day_balanced_mean_pct": np.nan,
            "ci_low_pct": np.nan,
            "ci_high_pct": np.nan,
        }

    daily = (
        selected.assign(
            _base=pd.to_numeric(
                selected["realized_base_net_return_pct"], errors="coerce"
            )
        )
        .groupby("trading_day")["_base"]
        .mean()
        .dropna()
        .to_numpy(dtype=float)
    )
    if len(daily) == 0:
        return {
            "days": 0,
            "day_balanced_mean_pct": np.nan,
            "ci_low_pct": np.nan,
            "ci_high_pct": np.nan,
        }

    rng = np.random.default_rng(20261104)
    indices = rng.integers(0, len(daily), size=(samples, len(daily)))
    bootstrap_means = daily[indices].mean(axis=1)
    return {
        "days": int(len(daily)),
        "day_balanced_mean_pct": float(daily.mean()),
        "ci_low_pct": float(np.quantile(bootstrap_means, 0.025)),
        "ci_high_pct": float(np.quantile(bootstrap_means, 0.975)),
    }


def success_check(
    details: pd.DataFrame,
    diagnostics: pd.DataFrame,
    bootstrap: dict[str, float | int],
) -> dict[str, bool]:
    baseline = details.loc[
        details["policy"].eq("earliest_ev_cap1")
    ].set_index("month")
    waited = details.loc[
        details["policy"].eq("wait_value_cap1")
    ].set_index("month")
    common_months = sorted(set(baseline.index) & set(waited.index))

    enough_trades = (
        len(common_months) == 3
        and waited.loc[common_months, "trades"].ge(MIN_MONTH_TRADES).all()
    )
    base_positive = (
        len(common_months) == 3
        and waited.loc[common_months, "base_mean_pct"].gt(0).all()
    )
    day_positive = (
        len(common_months) == 3
        and waited.loc[
            common_months, "day_balanced_base_mean_pct"
        ].gt(0).all()
    )
    improves_day = (
        len(common_months) == 3
        and (
            waited.loc[
                common_months, "day_balanced_base_mean_pct"
            ]
            > baseline.loc[
                common_months, "day_balanced_base_mean_pct"
            ]
        ).all()
    )
    tail_not_worse = (
        len(common_months) == 3
        and (
            waited.loc[common_months, "base_p05_pct"]
            >= baseline.loc[common_months, "base_p05_pct"]
        ).all()
        and (
            waited.loc[common_months, "stress_mean_pct"]
            >= baseline.loc[common_months, "stress_mean_pct"]
        ).all()
    )

    ev_diag = diagnostics.loc[
        diagnostics["pool"].eq("ev_qualified")
    ].set_index("month")
    positive_spearman = (
        len(common_months) == 3
        and set(common_months).issubset(ev_diag.index)
        and ev_diag.loc[common_months, "spearman"].gt(0).all()
    )
    bootstrap_positive = bool(
        np.isfinite(float(bootstrap["ci_low_pct"]))
        and float(bootstrap["ci_low_pct"]) > 0
    )

    checks = {
        "enough_trades_every_month": bool(enough_trades),
        "base_positive_every_month": bool(base_positive),
        "day_positive_every_month": bool(day_positive),
        "improves_day_every_month": bool(improves_day),
        "tail_and_stress_not_worse_every_month": bool(tail_not_worse),
        "ev_pool_wait_spearman_positive_every_month": bool(
            positive_spearman
        ),
        "pooled_day_bootstrap_lower_bound_positive": bool(
            bootstrap_positive
        ),
    }
    checks["all_pass"] = all(checks.values())
    return checks


def render_report(
    details: pd.DataFrame,
    summary: pd.DataFrame,
    diagnostics: pd.DataFrame,
    comparisons: pd.DataFrame,
    bootstrap: dict[str, float | int],
    checks: dict[str, bool],
) -> str:
    check_frame = pd.DataFrame(
        [{"criterion": key, "pass": value} for key, value in checks.items()]
    )
    bootstrap_frame = pd.DataFrame([bootstrap])
    return "\n".join(
        [
            "=== MoneyMaker State Wait Value v0.4 ===",
            "buy_gate=calibrated direct best base EV >= 0.50%",
            "wait_target=future 15m best realized action value minus current best realized action value",
            "wait_policy=first EV-qualified state with calibrated predicted wait advantage <= 0.00%; one attempt per ticker-day",
            "features=point-in-time state + breadth + 36 causal ticker-local lags",
            "evaluation=leave-one-month-out January-March 2026",
            "NOTE=development diagnostic only; no fresh month consumed",
            "",
            "=== Cross-month summary ===",
            summary.to_string(index=False),
            "",
            "=== Month-by-month ===",
            details.to_string(index=False),
            "",
            "=== Holdout wait-value diagnostics ===",
            diagnostics.to_string(index=False),
            "",
            "=== Common-episode comparison ===",
            comparisons.to_string(index=False),
            "",
            "=== Pooled day-cluster bootstrap ===",
            bootstrap_frame.to_string(index=False),
            "",
            "=== Pre-registered success check ===",
            check_frame.to_string(index=False),
        ]
    )


def _parse_dataset(value: str) -> tuple[str, Path]:
    if "=" not in value:
        raise argparse.ArgumentTypeError("--dataset must be LABEL=PATH")
    label, path = value.split("=", 1)
    return label.strip(), Path(path)


def main() -> int:
    parser = argparse.ArgumentParser(
        prog="python -m victory_trader.state_wait_value"
    )
    parser.add_argument(
        "--dataset", action="append", type=_parse_dataset, required=True
    )
    parser.add_argument("--report", type=Path, required=True)
    parser.add_argument("--details-csv", type=Path, required=True)
    parser.add_argument("--trades-csv", type=Path, required=True)
    parser.add_argument("--summary-csv", type=Path, required=True)
    parser.add_argument("--diagnostics-csv", type=Path, required=True)
    parser.add_argument("--comparisons-csv", type=Path, required=True)
    args = parser.parse_args()

    monthly = {
        label: pd.read_parquet(path)
        for label, path in args.dataset
    }
    details, trades, diagnostics, comparisons = run_lomo(monthly)
    summary = summarize(details)
    bootstrap = day_cluster_bootstrap(trades)
    checks = success_check(details, diagnostics, bootstrap)
    report = render_report(
        details,
        summary,
        diagnostics,
        comparisons,
        bootstrap,
        checks,
    )
    print(report)

    for path in (
        args.report,
        args.details_csv,
        args.trades_csv,
        args.summary_csv,
        args.diagnostics_csv,
        args.comparisons_csv,
    ):
        path.parent.mkdir(parents=True, exist_ok=True)
    args.report.write_text(report + "\n", encoding="utf-8")
    details.to_csv(args.details_csv, index=False)
    trades.to_csv(args.trades_csv, index=False)
    summary.to_csv(args.summary_csv, index=False)
    diagnostics.to_csv(args.diagnostics_csv, index=False)
    comparisons.to_csv(args.comparisons_csv, index=False)
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
